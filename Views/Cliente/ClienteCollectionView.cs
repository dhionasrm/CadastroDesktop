...
namespace CadastroDesktop.Views.ClienteCollectionView{
    public partial class ClienteCollectionView : XtraUserControl {
        public ClienteCollectionView() {
            InitializeComponent();
			if(!mvvmContext.IsDesignMode)
                InitBindings();
        }
        void InitBindings() {
            var fluentAPI = mvvmContext.OfType<CadastroDesktop.ViewModels.ClienteCollectionViewModel>();
            ...
            fluentAPI.WithEvent<GridView, FocusedRowObjectChangedEventArgs>(gridView, "FocusedRowObjectChanged")
                .SetBinding(x => x.SelectedEntity,
                    args => args.Row as CadastroDesktop.Modelo.Cliente,
